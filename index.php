<?php
  session_start();
  if(isset($_SESSION["cliente"])){
    $nome_cliente = $_SESSION["cliente"][1];
    $telefone_cliente = $_SESSION["cliente"][2];
    $email_cliente = $_SESSION["cliente"][3];
    $endereco_cliente = $_SESSION["cliente"][4];
    $tem_sessao = true;
  } else {
    $tem_sessao = false;
  }
?>

<!DOCTYPE html>
<html>
  <head>
    <title>Atelier Dyanne <?php 
      if($tem_sessao){
        echo " - ".$nome_cliente;
      }
    ?></title>
    <link rel="shortcut icon" href="atelier.ico" type="image/x-icon" />
    <meta charset="UTF-8" />
    <link
      rel="stylesheet"
      href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"
    />
    <link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css"
    />
    <link rel="stylesheet" href="./pagina.css" />
  </head>
  <body>
    <!--barra de navegação-->
    <?php require_once("./partials/_navbar.php") ?>
    <!--  capa da página  -->

    <div class="capa">
      <div class="texto-capa">
        <h1><img src="./img/logo.png" alt="" /></h1>
        <br />
        <?php
          if($tem_sessao):
        ?>
        <h2>
          Seja bem-vindo <?php echo $nome_cliente ?>. <br />
          Esperamos que tenha uma experiência maravilhosa com a nossa companhia.
        </h2>
        <?php endif; ?>
        <?php
          if(!$tem_sessao):
        ?>
        <h2>
          Seja bem-vindo a nossa página. <br />
          Esperamos que tenha uma experiência maravilhosa com a nossa companhia.
        </h2>
        <a href="./cadastro.php" class="btn btn-lg btn-ideia"
          >Criar uma conta</a
        >
        <a href="./login.php" class="btn btn-lg btn-ideia2">Entrar</a>
        <?php endif; ?>
      </div>
    </div>
    <!--  conteúdo  -->
    <section id="sobre">
      <div class="container">
        <div class="row">
          <!-- imagens -->
          <div class="col-md-6">
            <div class="row" id="imagens">
              <div class="col-md-6">
                <img
                  src="./imagens/wallpaper_Easy-Resize.com.jpg"
                  alt=""
                  style="border-radius: 10px"
                />
              </div>
              <div class="col-md-6">
                <img
                  src="./imagens/avent11_Easy-Resize.com (1).jpg"
                  alt=""
                  style="border-radius: 10px"
                />
              </div>
            </div>
            <div class="row" id="imagens">
              <div class="col-md-6">
                <img
                  src="./imagens/brinco6_Easy-Resize.com.jpg"
                  alt=""
                  style="border-radius: 10px"
                />
              </div>
              <div class="col-md-6">
                <img
                  src="./imagens/tiara2_Easy-Resize.com.jpg"
                  alt=""
                  style="border-radius: 10px"
                />
              </div>
            </div>
          </div>
          <!-- fim imagens -->

          <!-- texto -->
          <div class="col-md-6">
            <h2 style="color: #cca152; text-align: center">
              O que o Atelier Dyanne tem?
            </h2>
            <h3 style="color: #cca152">Vestidos de Noivas e acessórios</h3>
            <p>
              O Atelier Dyanne tem diversidade de produtos para o gosto de todas
              as mulheres, combinações perfeitas para noivas e damas de honra
            </p>

            <h3 style="color: #cca152">Diversidade de acessórios e bolsas</h3>
            <p>
              O Atelier Dyanne oferece aos seus clientes acessórios e bolsas
              para usar e desfrutar no seu dia-a-dia, aqui você pode encontrar o
              acesssório ideal para compartilhar com quem ama ou estar
              deslumbrante em qualquer evento.
            </p>

            <h3 style="color: #cca152">Serviços mais solicitados</h3>
            <p>
              O Atelier Dyanne é a escolha Nº1 de muitos noivos e padrinhos.
              Oferecendo sempre Serviços de qualidade feitos com muito amor.
            </p>
          </div>
        </div>
      </div>
    </section>

    <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.5.4/dist/umd/popper.min.js"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
  </body>
</html>
